                                      ...
                                      vptr_t bi_frame_vptr, word_t num_bi_pages)
{
    ...
    assert(0 == (rootserver.boot_info % BIT(seL4_PageBits)));

    for (word_t i = 0; i < num_bi_pages; i++) {
        /* Create a cap for a non-executable page. We cannot control the
         * read/write access here and make is read-only for the initial thread.
         */
        word_t offset = i * BIT(seL4_PageBits);
        cap_t bi_page_cap = create_mapped_it_frame_cap(
                                it_vspace_cap,
                                rootserver.boot_info + offset,
        ...
        if (1 == i) {
            ndks_boot.bi_frame->extraBIPages.start = ndks_boot.slot_pos_cur;
        }
        ...
        ndks_boot.bi_frame->extraBIPages.end = ndks_boot.slot_pos_cur;
    }
