...
BOOT_BSS static p_region_t reg_reserved[MAX_NUM_FREEMEM_REG + NUM_RESERVED_REGIONS];
...
BOOT_CODE static cap_t create_root_cnode(void)
{
...
BOOT_CODE bool_t create_untypeds(cap_t root_cnode_cap)
{
    ...
    paddr_t start = 0;
    for (word_t i = 0; i < ARRAY_SIZE(reg_reserved); i++) {
        p_region_t *p_reg = &reg_reserved[i];
        if (is_p_reg_empty(*p_reg)) {
            continue;
        }
        paddr_t reg_start = p_reg->start;
        if (start < reg_start) {
            p_region_t reg = {
                .start = start,
                .end = reg_start
    ...
    for (int i = 0; i < ARRAY_SIZE(reg_freemem); i++) {
        ...
        if (!create_untypeds_for_phys_region(root_cnode_cap, false, reg, first_untyped_slot)) {
            printf("ERROR: creation of untypeds for free memory region #%d at"
                   " [%"SEL4_PRIx_word"..%"SEL4_PRIx_word"] failed\n",
                   i, reg.start, reg.end);
            return false;
        }
...
{
    /* Ensure reg_freemem is properly initialized. */
    ...
    for (word_t i = 0; i < n_available; i++) {
        ...
        if (cnt >= ARRAY_SIZE(reg_freemem)) {
             ...
             */
            printf("    WARNING: can't use region, reg_freemem[] is full\n");
            assert(0);
            continue;
        }

        reg_freemem[cnt] = usable_reg;
        cnt++;
    }
...
{
    /* Check the available memory region and initialize reg_freemem to hold the
    ...
    /* There is no need to check the reserved region, because reserve_region()
     * has taken case of this.
     */
    ...
    while ((idx_a < n_available) || (!is_p_reg_empty(a_chunk))) {
        ...
        if (!is_reserved_slot_used(idx_r)) {
            ...
            if (r->end <= a->start) {
                ...
                if (r->end < a->end) {
                     ...
                     * next available region in the next loop iteration.
                     */
                    a_chunk = P_REG_EMPTY;
                    idx_a++;
                }
         ...
         * create device untyped caps for all non-reserved region later.
         */
    ...
    /* Now reg_freemem[] is set up to contain the usable memory. First thing
