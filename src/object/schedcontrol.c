...
static exception_t decodeSchedControl_ConfigureFlags(word_t length, cap_t cap, word_t *buffer)
{
    ...
    if (_from_time_t(budget_us) > _from_time_t(MAX_PERIOD_US) || budget_ticks < MIN_BUDGET_TICKS) {
        userError("SchedControl_ConfigureFlags: budget out of range.");
        current_syscall_error.type = seL4_RangeError;
        current_syscall_error.rangeErrorMin = _from_time_t(MIN_BUDGET_US);
        current_syscall_error.rangeErrorMax = _from_time_t(MAX_PERIOD_US);
        return EXCEPTION_SYSCALL_ERROR;
    }

    if (_from_time_t(period_us) > _from_time_t(MAX_PERIOD_US) || period_ticks < MIN_BUDGET_TICKS) {
    ...
    if (budget_ticks > period_ticks) {
        ...
        current_syscall_error.type = seL4_RangeError;
        current_syscall_error.rangeErrorMin = _from_time_t(MIN_BUDGET_US);
        current_syscall_error.rangeErrorMax = _from_time_t(period_us);
        return EXCEPTION_SYSCALL_ERROR;
    }
