...
void VISIBLE NORETURN c_handle_interrupt(int irq, int syscall)
{
    ...
    if (irq == int_unimpl_dev) {
        ...
        trace_kernel_entry(Entry_UnimplementedDevice, irq);
#endif
...
#ifdef TRACK_KERNEL_ENTRIES
        trace_kernel_entry(Entry_VMFault, type);
#endif
        ...
        /* Debug exception */
#ifdef TRACK_KERNEL_ENTRIES
        trace_kernel_entry(Entry_DebugFault, 
...
#ifdef TRACK_KERNEL_ENTRIES
        trace_kernel_entry(Entry_UserLevelFault, irq);
#endif
...
#ifdef TRACK_KERNEL_ENTRIES
        trace_kernel_entry(Entry_Interrupt, irq);
#endif
...
#ifdef TRACK_KERNEL_ENTRIES
        trace_kernel_entry(Entry_UnknownSyscall, sys_num);
#endif
...
#ifdef TRACK_KERNEL_ENTRIES
    trace_kernel_entry_syscall(cptr, msgInfo, syscall, 1);
#endif
...
{
#ifdef TRACK_KERNEL_ENTRIES
    trace_kernel_entry(Entry_VMExit, 0);
#endif
