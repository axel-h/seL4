...
def get_addrspace_exclude(regions: List[Region], config: Config):
    ...
    ret = set()
    # We can't create untypeds that exceed the the physical address space, so we
    # round down to the smallest untyped size alignment so that the kernel will
    # be able to turn the entire range into untypeds.
    as_max = hardware.utils.align_down(2**config.phys_addr_space_bits,
