...
#define MIN_BUDGET_US _as_time_t(2u * _from_time_t(getKernelWcetUs()) * CONFIG_KERNEL_WCET_SCALE)
#define MIN_BUDGET_TICKS (2u * getKernelWcetTicks() * CONFIG_KERNEL_WCET_SCALE)
#if (CONFIG_KERNEL_STATIC_MAX_PERIOD_US) != 0
#define MAX_PERIOD_US _as_time_t(CONFIG_KERNEL_STATIC_MAX_PERIOD_US)
#else
 ...
 * than 2^62. */
#define MAX_PERIOD_US _as_time_t( _from_time_t(getMaxUsToTicks()) / 8)
...
{
    return refill_head(sc)->rTime <= NODE_STATE(ksCurTicks);
}
