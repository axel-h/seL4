...
static inline void benchmark_utilisation_switch(tcb_t *heir, tcb_t *next)
{
    ...
    if (likely(NODE_STATE(benchmark_log_utilisation_enabled))) {
        ...
        if (likely(ksTimestampEnter > heir->benchmark.schedule_start_time)) {

            heir->benchmark.utilisation += (ksTimestampEnter - heir->benchmark.schedule_start_time);

        } else {
#ifdef CONFIG_ARM_ENABLE_PMU_OVERFLOW_INTERRUPT
            heir->benchmark.utilisation += (UINT32_MAX - heir->benchmark.schedule_start_time) + ksTimestampEnter;
            armv_handleOverflowIRQ();
#endif /* CONFIG_ARM_ENABLE_PMU_OVERFLOW_INTERRUPT */
        }

        /* Reset next thread utilisation */
        next->benchmark.schedule_start_time = ksTimestampEnter;
...
static inline void benchmark_utilisation_finalise(void)
{
    ...
    NODE_STATE(benchmark_end_time) = ksTimestampEnter;
